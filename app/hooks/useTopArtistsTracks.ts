...
//   : never
export type ArtistPagingObject = SpotifyApi.PagingObject<SpotifyApi.ArtistObjectFull>
export type TrackPagingObject = SpotifyApi.PagingObject<SpotifyApi.TrackObjectFull>
...
export const useTopArtistsTracks = ({ limit }: OptionsPropTopArtistsTracks = { limit: 50 }) => {
  ...
  const { refetch: refetchArtists, ...resultArtists } = useSpotify<ArtistPagingObject>({
    url: urlArtists,
  })
  const { refetch: refetchTracks, ...resultTracks } = useSpotify<TrackPagingObject>({
    url: urlTracks,
  })
